...
async def notify_subscribers(ctx, listing_id: int, action: str, ltype: str, lat: float, lon: float, price: float):
    ...
    for user_id, stype, sub_lat, sub_lon, radius, max_price in subscriptions:
        ...
        try:
            ...
            buttons.append([
                InlineKeyboardButton("🗺 На карте",   callback_data=f"map_{listing_id}"),
                InlineKeyboardButton("✉️ Написать",  callback_data=f"contact_{listing_id}_{owner_id}")
            ])
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for row in no_geo:
        ...
        buttons.append([InlineKeyboardButton("✉️ Написать", callback_data=f"contact_{lid}_{owner_id}")])
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            CONTACT_MSG: [
                MessageHandler(filters.Regex("^🏠 На главную$"), home_button_pressed),
                MessageHandler(filters.TEXT & ~filters.COMMAND, contact_send),
            ],
            # Админ
            ADMIN_MENU: [
                CallbackQueryHandler(go_home,        pattern="^go_home$"),
