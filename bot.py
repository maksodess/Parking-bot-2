...
async def _show_ad_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Показываем ВСЕ фото медиагруппой + превью отдельным сообщением
    if len(photos) > 1:
        from telegram import InputMediaPhoto
...
def main():
    ...
    logger.info("🤖 ParkRent Varna запущен!")
    app.run_polling()
