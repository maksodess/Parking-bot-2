...
async def search_location_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "sloc_text":
        ...
        await query.edit_message_text(
            "📍 Изпратете вашу геолокацию:\n\n"
            "• Нажмите кнопку «Изпращане на моята геолокация» по-долу\n"
            "• Или чрез скрепку 📎 → Геолокация → намерете место на карте → Изпращане"
        )
        await query.message.reply_text("Изберете способ:", reply_markup=geo_search_keyboard())
        return SEARCH_GEO
