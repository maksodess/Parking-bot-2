...
Геокодинг через OpenStreetMap Nominatim (бесплатно)
"""
...
(
    MAIN_MENU,
    AD_TYPE, AD_ADDRESS, AD_ADDRESS_CONFIRM, AD_PHONE,
    AD_PRICE, AD_DESCRIPTION, AD_PHOTO, AD_CONFIRM,
    SEARCH_TYPE, SEARCH_ADDRESS, SEARCH_RADIUS,
    CONTACT_MSG,
...
def geocode(address: str):
    ...
    Добавляем ', Варна, Болгария' для точности.
    """
...
# ── Лейблы ────────────────────────────────────────────────────
ACTION_LABEL = {"buy":"🛒 Купить","sell":"💰 Продать","rent":"🔑 Арендовать","lease":"📋 Сдать в аренду"}
TYPE_LABEL   = {"parking":"🅿️ Паркоместо","garage":"🏠 Гараж","all":"📋 Всё"}
...
async def main_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📁 Мои объявления":
        ...
        await update.message.reply_text(
            ...
            "📍 *Адрес* — просто введите улицу и номер дома,\n"
            "бот сам найдёт координаты на карте.\n\n"
            "🔍 *Поиск по радиусу* — введите свой адрес или\n"
            "отправьте геолокацию, выберите радиус.",
...
async def ad_type_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Например: _ул. Цар Симеон I, 15_ или _бул. Владислав Варненчик 42_\n\n"
        "Бот автоматически найдёт это место на карте.",
        parse_mode="Markdown"
    )
    return AD_ADDRESS
...
async def ad_address(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        await update.message.reply_text(
            ...
            "Попробуйте указать точнее, например:\n"
            "_ул. Цар Симеон I 15_ или _бул. Приморски 42_",
            parse_mode="Markdown"
        )
        return AD_ADDRESS
    ...
    await update.message.reply_text(
        f"📍 Нашёл вот это место:\n_{display[:120]}_\n\n"
        f"Это правильный адрес?",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Да, верно",       callback_data="addr_ok")],
            [InlineKeyboardButton("❌ Нет, ввести снова", callback_data="addr_retry")],
        ])
    )
...
async def search_type_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "📍 Введите *ваш адрес* для поиска рядом с вами\n\n"
        "Например: _ул. Цар Симеон I 15_\n\n"
        "Или нажмите кнопку ниже чтобы отправить геолокацию автоматически.",
        parse_mode="Markdown"
    )
    await query.message.reply_text(
        "Введите адрес или отправьте геолокацию:",
        reply_markup=geo_search_keyboard()
    )
    return SEARCH_ADDRESS

async def search_address_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    rl = f"{radius//1000} км" if radius and radius >= 1000 else f"{radius} м" if radius else "вся Варна"
    await query.edit_message_text(
        f"🔍 Найдено *{len(results)}* объявл. · радиус: {rl}",
        parse_mode="Markdown"
    )
...
async def show_my_listings(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return MAIN_MENU
    await update.message.reply_text(
        f"📁 Ваши объявления ({len(rows)}):", reply_markup=home_ikb()
    )
    for row in rows:
        ...
        if row[11]:
            await update.message.reply_photo(row[11], caption=caption, parse_mode="Markdown",
...
async def admin_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return MAIN_MENU
    return await send_admin_menu(update.message.reply_text)

async def send_admin_menu(send_fn):
    conn = db()
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            AD_ADDRESS_CONFIRM: [CallbackQueryHandler(ad_address_confirm, pattern="^addr_")],
            ...
            SEARCH_ADDRESS: [
                ...
                MessageHandler(filters.TEXT & ~filters.COMMAND, search_address_input),
            ],
            SEARCH_RADIUS: [
