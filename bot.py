...
async def send_listing(message, caption: str, row, keyboard):
    """Отправить объявление: фото → текст → кнопки (3 сообщения)."""
    ...
    # Затем кнопки отдельным сообщением
...
async def _show_ad_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Три отдельных сообщения: фото → текст → кнопки
    if photos:
        # Сначала фото
        if len(photos) == 1:
    ...
    # Затем текст
    await update.effective_message.reply_text(preview, parse_mode="Markdown")
