...
def has_purchased_contacts(buyer_id: int, listing_id: int) -> bool:
    """Проверяет купил ли пользователь доступ к контактам этого объявления."""
    conn = db()
    result = conn.execute(
...
def listing_text(row, distance_m=None, viewer_id=None, show_contacts=False):
    """
    Формирует текст объявления.
    
    viewer_id: ID пользователя который смотрит объявление
    show_contacts: True если контакты уже оплачены/доступны
    """
    ...
    is_owner = viewer_id and viewer_id == owner_id
    is_admin = viewer_id and viewer_id == ADMIN_ID
    contacts_visible = is_owner or is_admin or show_contacts
    
    # Адрес — показываем только район если контакты скрыты
    if contacts_visible:
        lines.append(f"📍 {address}")
    else:
        # Показываем только район (последняя часть после запятой) или первое слово
        if ',' in address:
            # Берём последнюю часть — обычно это район
            short_addr = address.split(',')[-1].strip()
        else:
            # Если нет запятых — берём первое слово
            short_addr = address.split()[0] if address else "Варна"
        lines.append(f"📍 ~{short_addr}")
    
    # Телефон только если контакты видимы
    if phone and contacts_visible:
        lines.append(f"📞 {phone}")
    elif phone and not contacts_visible:
        lines.append(f"📞 Номер скрыт")
...
def action_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📁 Мои объявления", callback_data="start_mylistings")],
    ])
...
def phone_keyboard():
    """Клавиатура с кнопкой автоматической отправки своего номера."""
    return ReplyKeyboardMarkup([
...
async def start_action(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "mylistings":
        ...
        conn.close()
        if not rows:
            await query.edit_message_text("У вас нет объявлений.", reply_markup=home_ikb())
            return MAIN_MENU
        await query.edit_message_text(f"📁 Ваши объявления ({len(rows)}):", reply_markup=home_ikb())
        for row in rows:
            lid, active = row[0], row[12]
            status  = "✅ Активно" if active else "⏸ Снято"
            caption = listing_text(row, viewer_id=user_id, show_contacts=True) + f"\n{status}"
            btns    = []
            if active:
...
async def ad_address_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        ...
        "Нажмите кнопку чтобы отправить свой номер автоматически,\n"
...
async def ad_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("✏️ Изменить адрес",      callback_data="ad_edit_address"),
             InlineKeyboardButton("✏️ Изменить телефон",    callback_data="ad_edit_phone")],
            [InlineKeyboardButton("✏️ Изменить цену",       callback_data="ad_edit_price"),
             InlineKeyboardButton("✏️ Изменить описание",   callback_data="ad_edit_desc")],
            [InlineKeyboardButton("✏️ Изменить фото",       callback_data="ad_edit_photo")],
...
async def ad_edit_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "ad_edit_address":
        await query.edit_message_text(
            "📍 Как указать новое местоположение?",
            reply_markup=location_choice_keyboard("adloc")
        )
        return AD_LOCATION_CHOICE
    
    elif query.data == "ad_edit_phone":
        await query.edit_message_text("📞 Номер телефона для связи")
        await query.message.reply_text(
        ...
        await query.edit_message_text("💰 Введите новую *цену* (число, €):", parse_mode="Markdown")
        return AD_PRICE
...
async def notify_subscribers(ctx, listing_id: int, action: str, ltype: str, lat: float, lon: float, price: float):
    """Отправляет уведомления подписчикам о новом объявлении."""
    ...
    # Получаем само объявление для показа
    ...
    for user_id, stype, sub_lat, sub_lon, radius, max_price in subscriptions:
        ...
        try:
            owner_id = listing[1]
            photo_id = listing[11]
            purchased = has_purchased_contacts(user_id, listing_id)
            caption = listing_text(listing, distance_m=dist, viewer_id=user_id, show_contacts=purchased)
            notification = f"🔔 *Новое объявление по вашей подписке!*\n\n{caption}"
            
            # Кнопки как в обычном поиске
            buttons = []
            if not purchased and user_id != owner_id and user_id != ADMIN_ID:
                buttons.append([InlineKeyboardButton("📞 Показать контакты (2€)", callback_data=f"reveal_{listing_id}")])
            
            buttons.append([InlineKeyboardButton("🗺 На карте", callback_data=f"map_{listing_id}")])
...
async def search_location_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "sloc_text":
        ...
        await query.edit_message_text(
            ...
            "• Или через скрепку 📎 → Геолокация → найдите место на карте → Отправить"
        )
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if total == 0:
        ...
        ctx.user_data["subscribe_params"] = {
            ...
            "lat": user_lat,
            "lon": user_lon,
            "radius": radius or 50000,  # Вся Варна = 50км
        }
        
        await query.edit_message_text(
            f"😕 В радиусе {rl} объявлений не найдено.\n\n"
            f"💡 *Хотите получить уведомление* когда появится подходящее объявление?",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔔 Подписаться на уведомления (5€/мес)", callback_data="subscribe")],
    ...
    for dist, row in results:
        ...
        purchased = has_purchased_contacts(viewer_id, lid)
        caption  = listing_text(row, distance_m=dist, viewer_id=viewer_id, show_contacts=purchased)
        
        buttons = []
        # Кнопка "Показать контакты" если не куплено и не владелец
        if not purchased and viewer_id != owner_id and viewer_id != ADMIN_ID:
            buttons.append([InlineKeyboardButton("📞 Показать контакты (2€)", callback_data=f"reveal_{lid}")])
        
        buttons.append([InlineKeyboardButton("🗺 На карте", callback_data=f"map_{lid}")])
    ...
    for row in no_geo:
        ...
        viewer_id = query.from_user.id
        purchased = has_purchased_contacts(viewer_id, lid)
        caption  = listing_text(row, viewer_id=viewer_id, show_contacts=purchased)
        
        buttons = []
        if not purchased and viewer_id != owner_id and viewer_id != ADMIN_ID:
            buttons.append([InlineKeyboardButton("📞 Показать контакты (2€)", callback_data=f"reveal_{lid}")])
        
        # Для объявлений без геолокации кнопка карты не нужна, но оставим пустую кнопку или вообще без кнопок
...
async def show_map(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    full_row = conn.execute("SELECT * FROM listings WHERE id=?", (lid,)).fetchone()
    conn.close()
    ...
    owner_id = full_row[1]
    lat, lon = full_row[7], full_row[8]
    address = full_row[5]
    
    # Проверяем доступ к контактам
    is_owner = viewer_id == owner_id
    is_admin = viewer_id == ADMIN_ID
    purchased = has_purchased_contacts(viewer_id, lid)
    
    if not (is_owner or is_admin or purchased):
        await query.answer(
            "📍 Точное местоположение доступно после оплаты контактов",
            show_alert=True
        )
        return MAIN_MENU
...
async def reveal_contacts(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Обработка кнопки 'Показать контакты'."""
    query = update.callback_query
    await query.answer()
    lid = int(query.data.split("_")[1])
    ...
    if has_purchased_contacts(buyer_id, lid):
        await query.answer("Вы уже купили доступ к этим контактам!", show_alert=True)
        return MAIN_MENU
    
    # TODO: Здесь будет интеграция с Telegram Payments
    # Пока просто сохраняем покупку бесплатно для тестирования
    ...
    # Получаем объявление с полными контактами
    row = conn.execute("SELECT * FROM listings WHERE id=?", (lid,)).fetchone()
    conn.close()
    
    if row:
        owner_id = row[1]
        caption = listing_text(row, viewer_id=buyer_id, show_contacts=True)
        
        # Обновляем caption с новыми кнопками (без "Написать", только "На карте")
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🗺 На карте", callback_data=f"map_{lid}")],
        ])
        
        await query.edit_message_caption(
            caption=f"✅ Контакты открыты!\n\n{caption}",
            parse_mode="Markdown",
            reply_markup=keyboard
        )
        await query.answer("✅ Контакты открыты! (тестовый режим — оплата пока бесплатная)", show_alert=True)
...
async def subscribe_to_notifications(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not params:
        await query.answer("❌ Параметры поиска потеряны, повторите поиск", show_alert=True)
        return MAIN_MENU
    ...
    if existing:
        await query.edit_message_text(
            "У вас уже есть активная подписка с этими параметрами!",
    ...
    await query.edit_message_text(
        ...
        f"🔔 Вы получите уведомление когда появится подходящее объявление.\n\n"
        f"⏰ Подписка действует до: {expires[:10]}\n\n"
        f"_Тестовый режим — подписка бесплатная_",
        parse_mode="Markdown",
        reply_markup=home_ikb()
    )
...
async def fix_addresses_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Админская команда: переопределяет адрес для всех объявлений где address выглядит как координаты."""
    ...
    conn.commit()
    conn.close()
    await update.message.reply_text(f"✅ Готово! Обновлено объявлений: {fixed}")
    return MAIN_MENU
    ...
    conv = ConversationHandler(
        ...
        states={
            MAIN_MENU: [
                ...
                CallbackQueryHandler(reveal_contacts,           pattern="^reveal_"),
            ...
            AD_ADDRESS_TEXT: [
                MessageHandler(filters.Regex("^🏠 На главную$"), home_button_pressed),
