...
import json
import re
from translations import t, get_user_language, set_user_language
...
def action_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📁 Мои объявления", callback_data="start_mylistings"),
         InlineKeyboardButton("⭐ Избранное",       callback_data="start_favorites")],
        [InlineKeyboardButton("🔔 Мои подписки",   callback_data="start_subscriptions")],
    ])
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if total == 0:
        ...
        await query.edit_message_text(
            ...
            f"💡 Хотите получать уведомления когда появится подходящее объявление?",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔔 Подписаться на уведомления (⭐100 ≈ 2€)", callback_data="subscribe")],
    ...
    for dist, row in results:
        ...
        fav_text = "💔 Удалить из избранного" if in_favorites else "⭐ В избранное"
...
async def successful_payment_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if parts[0] == "contacts":
            ...
            # Получаем параметры из user_data (lat, lon)
            params = ctx.user_data.get("subscribe_params")
            if not params:
                await update.message.reply_text("❌ Параметры подписки потеряны. Повторите поиск.")
                return
            ...
            conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (user_id, search_type, action, params["lat"], params["lon"], radius, expires)
            )
            conn.commit()
            conn.close()
            
            radius_text = f"{radius//1000} км" if radius >= 1000 else f"{radius} м"
            type_text = TYPE_LABEL.get(search_type, search_type)
            action_text = ACTION_LABEL.get(action, action)
...
async def subscribe_to_notifications(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Обработка подписки на уведомления - запуск оплаты."""
    query = update.callback_query
    await query.answer()
    ...
    # Формируем инвойс для оплаты подписки
    ...
    description = f"{action_text}, {type_text}, радиус {radius_text}"
    payload = f"subscription_{user_id}_{params['search_type']}_{params['action']}_{params['radius']}"
    stars_price = 100  # 100 Stars за месяц подписки
    ...
    try:
        await query.message.reply_invoice(
            ...
            start_parameter=f"subscribe_{user_id}",
        )
        await query.answer("⭐ Оплатите подписку через Telegram Stars")
    except Exception as e:
        logger.error(f"Payment error: {e}")
        await query.answer("❌ Ошибка создания платежа. Попробуйте позже.", show_alert=True)
...
async def cmd_my(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await update.message.reply_text("У вас пока нет объявлений.", reply_markup=home_ikb())
        return MAIN_MENU
    
    await update.message.reply_text(f"📁 *Ваши объявления* ({len(rows)})", parse_mode="Markdown")
    ...
    for row in rows:
        ...
        buttons = []
        if active:
            buttons.append([InlineKeyboardButton("⏸ Деактивировать", callback_data=f"deactivate_{lid}")])
        else:
        ...
        if row[11]:
            await update.message.reply_photo(row[11], caption=f"{caption}\n\n{status}", parse_mode="Markdown", reply_markup=keyboard)
        else:
...
async def cmd_subscriptions(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await update.message.reply_text("🔔 У вас нет подписок.", reply_markup=home_ikb())
        return MAIN_MENU
    
    await update.message.reply_text(f"🔔 Ваши подписки ({len(rows)}):", parse_mode="Markdown")
...
def main():
    ...
    conv = ConversationHandler(
        ...
        fallbacks=[
            CommandHandler("start", start),
            CommandHandler("cancel", start),  # Отмена = возврат на главную
            CommandHandler("admin", admin_cmd),
            CommandHandler("fixaddresses", fix_addresses_cmd),
        ],
        allow_reentry=True,
    ...
    from telegram.ext import PreCheckoutQueryHandler, MessageHandler
