...
async def _show_ad_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Показываем как в send_listing: ВСЕ фото в одной медиагруппе
    if not photos:
        # Нет фото
        await ctx.bot.send_message(chat_id=chat_id, text=preview, parse_mode="Markdown", reply_markup=keyboard)
    elif len(photos) == 1:
        # Одно фото с кнопками
        await ctx.bot.send_photo(chat_id=chat_id, photo=photos[0], caption=preview, parse_mode="Markdown", reply_markup=keyboard)
    else:
        # 2-5 фото — медиагруппа с текстом на первом
        from telegram import InputMediaPhoto
        media = [InputMediaPhoto(media=photos[0], caption=preview, parse_mode="Markdown")]
        for p in photos[1:5]:
            media.append(InputMediaPhoto(media=p))
        await ctx.bot.send_media_group(chat_id=chat_id, media=media)
        # Кнопки сразу после — ВАЖНО использовать bot.send_message
        await ctx.bot.send_message(chat_id=chat_id, text="​", reply_markup=keyboard)
