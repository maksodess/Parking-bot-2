...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for dist, row in results:
        ...
        # Первое сообщение убирает кнопку геолокации
        remove_kb = ReplyKeyboardRemove() if first else None
        first = False

        try:
            if row[11]:
                await query.message.reply_photo(
                    row[11], caption=caption, parse_mode="Markdown",
                    reply_markup=keyboard if not remove_kb else keyboard
                )
                if remove_kb:
                    pass  # фото не поддерживает ReplyKeyboardRemove — отправим ниже
            else:
                await query.message.reply_text(
            ...
            try:
                ...
                logger.error(f"Error listing {lid} no markdown: {e2}")
    ...
    for row in no_geo:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error no_geo listing {lid} no markdown: {e2}")
