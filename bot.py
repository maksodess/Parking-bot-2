...
“*ул. Цар Симеон I 15* или *бул. Владислав Варненчик 42*”,
parse_mode=“Markdown”
)
return AD_ADDRESS_TEXT
...
messages = {
    "price": f"💰 *Цена изменена*\nБыло: {old_value:,.0f} €\nСтало: {new_value:,.0f} €",
    "address": f"📍 *Адрес изменен*\nБыло: {old_value or '—'}\nСтало: {new_value or '—'}",
    "phone": f"📞 *Телефон изменен*\n{'Добавлен' if old_value is None else 'Изменен' if new_value else 'Удален'}",
    "description": f"📝 *Описание изменено*\n{'Добавлено' if old_value is None else 'Изменено' if new_value else 'Удалено'}",
    "photo": f"📸 *Снимки обновлены*\n{f'{new_value} фото' if new_value != 'removed' else 'Снимки удалены'}",
}
...
for (user_id,) in favorites_users:
    try:
        await bot.send_message(
            user_id,
            f"⭐ *Обявата из вашите любими е обновена!*\n\n"
            f"#{listing_id} · {listing[5] or '—'}\n\n"
            f"{notification_text}",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔍 Виж обявата", callback_data=f"view_{listing_id}")
            ]])
        )
        # Rate limiting
...
for (user_id,) in favorites_users:
    try:
        await bot.send_message(
            ...
            f"❌ *Обявата е изтрита*\n\n"
            f"#{listing_id} · {listing[5] or '—'}\n\n"
            f"Обявата беше премахната от вашите любими, тъй като е изтрита.",
            parse_mode="Markdown"
        )
        # Rate limiting
        await asyncio.sleep(0.05)
...
“/help — Помощ\n\n”
“*Обявите се изтриват автоматично ако не са потвърдени в рамките на 7 дни*”
)
