...
async def send_listing(message, caption: str, row, keyboard):
    ...
    if not photos:
        ...
        # Несколько фото: ПЕРВОЕ с текстом и кнопками, остальные медиагруппой
        await message.reply_photo(photos[0], caption=caption, parse_mode="Markdown", reply_markup=keyboard)
        # Остальные фото отдельно
...
async def _show_ad_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ПРОСТОЕ РЕШЕНИЕ: Одно фото с кнопками, остальные отдельно
    if not photos:
        await update.effective_message.reply_text(preview, parse_mode="Markdown", reply_markup=keyboard)
    elif len(photos) == 1:
        await update.effective_message.reply_photo(photos[0], caption=preview, parse_mode="Markdown", reply_markup=keyboard)
    else:
        # Первое фото С КНОПКАМИ
        await update.effective_message.reply_photo(photos[0], caption=preview, parse_mode="Markdown", reply_markup=keyboard)
        # Остальные фото медиагруппой
        if len(photos) > 1:
            from telegram import InputMediaPhoto
            media = [InputMediaPhoto(media=p) for p in photos[1:5]]
