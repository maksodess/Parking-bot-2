...
def geo_ad_keyboard():
    return ReplyKeyboardMarkup([
        [KeyboardButton("📍 Изпращане на геолокация объекта", request_location=True)],
...
def phone_keyboard():
    ...
    return ReplyKeyboardMarkup([
        [KeyboardButton("📱 Изпращане мой номер", request_contact=True)],
...
async def main_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏠 Начало":
        ...
        await update.message.reply_text(
            ...
            "💰 *Продава / Под наем* — разместить обявиение\n\n"
            "📍 *Местоположение объекта:*\n"
            "Можно ввести адрес текстом — бот найдёт его на карте.\n"
            "Или изпратете геолокация через 📎 → Геолокация.\n\n"
...
async def start_action(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "mylistings":
        ...
        conn.close()
        if not rows:
            await query.edit_message_text("У вас нет обявиений.", reply_markup=home_ikb())
            return MAIN_MENU
        await query.edit_message_text(f"📁 Ваши обявиения ({len(rows)}):", reply_markup=home_ikb())
        for row in rows:
            lid, active = row[0], row[12]
            status  = "✅ Активен" if active else "⏸ Снято"
    ...
    if action == "subscriptions":
        ...
        if not rows:
            await query.edit_message_text(
                ...
                "Подписки позволяют получать уведомления о новых обявиениях.",
...
async def ad_location_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "adloc_text":
        ...
        await query.edit_message_text(
            "📍 Изпратете геолокация объекта чрез бутона по-долу.\n\n"
...
async def ad_address_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "addrconfirm_retry":
        await query.edit_message_text("✏️ Въведете адрес снова:")
        return AD_ADDRESS_TEXT

    elif query.data == "addrconfirm_geo":
        await query.edit_message_text(
            "📍 Отправьте точна геолокацию через 📎 → Геолокация:"
        )
...
async def search_location_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "sloc_text":
        ...
        await query.edit_message_text(
            ...
            "• Нажмите кнопку «Изпращане на моята геолокация» по-долу\n"
            "• Или через скрепку 📎 → Геолокация → намерете место на карте → Изпращане"
        )
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if total == 0:
        ...
        await query.edit_message_text(
            ...
            f"💡 Хотите получать уведомления когда появится подходящее обявиение?",
            reply_markup=InlineKeyboardMarkup([
    ...
    try:
        ...
        if parts[0] == "contacts":
            ...
            if row:
                ...
                await update.message.reply_text("✅ Оплата прошла, но обявиение не найдено.")
...
async def subscribe_to_notifications(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"• {type_text}\n"
        f"• Радиус: {radius_text}\n\n"
        f"🔔 Вы получите уведомление когда появится подходящее обявиение.\n\n"
        f"⏰ Валиден: 30 дни",
        parse_mode="Markdown",
        reply_markup=home_ikb()
    )
...
async def show_my_listings(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not rows:
        await update.message.reply_text("У вас нет обявиений.", reply_markup=home_ikb())
        return MAIN_MENU
    await update.message.reply_text(f"📁 Ваши обявиения ({len(rows)}):", reply_markup=home_ikb())
    for row in rows:
        lid, active = row[0], row[12]
        status  = "✅ Активен" if active else "⏸ Снято"
        caption = listing_text(row) + f"\n{status}"
