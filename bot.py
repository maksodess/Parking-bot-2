...
async def send_listing(message, caption: str, row, keyboard):
    """Отправить объявление — все фото сразу одной медиагруппой."""
...
async def ad_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        # Telegram присылает медиагруппу как отдельные сообщения с одним media_group_id
        # Собираем все file_id пока приходят
        if file_id not in photos:
            photos.append(file_id)

        if len(photos) >= 5:
            # Достигли лимита — сразу показываем превью
            await update.message.reply_text(f"✅ {len(photos)} снимки получени!")
            return await _show_ad_preview(update, ctx)

        # Ждём ещё фото (медиагруппа) — показываем счётчик
        await update.message.reply_text(
            f"📸 Получени {len(photos)} снимки. Изпратете още или натиснете *Готово*:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"✅ Готово ({len(photos)} снимки)", callback_data="photos_done")],
            ])
        )
        return AD_PHOTO
...
async def _show_ad_preview(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✏️ Промяна на снимки",   callback_data="ad_edit_photo")],
    ...
    # Если есть фото — показываем первое с превью
    if photos:
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    from telegram import ReplyKeyboardRemove
    # Одно сообщение: убирает геолокацию И показывает счётчик
    await query.edit_message_text(
        f"🔍 Намерени *{total}* обяви · радиус: {rl}",
        parse_mode="Markdown"
    )
    # Убираем ReplyKeyboard (кнопку геолокации)
    await query.message.reply_text(
        "⬇️ Резултати:", reply_markup=ReplyKeyboardRemove()
    )
    ...
    for dist, row in results:
        ...
        try:
            ...
            logger.error(f"Error listing {lid}: {e}")
            try:
