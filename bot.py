...
ADMIN_ID  = 5053888378
DATABASE_URL = os.environ.get("DATABASE_URL", "")  # Railway добавит автоматически
PAGE_SIZE = 10
...
def db():
    """Короткий алиас для подключения."""
...
def listing_text(row, distance_m=None):
    """Формирует текст объявления (все контакты видны всем)."""
...
async def start_action(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "mylistings":
        ...
        conn = db()
        rows = conn.execute("SELECT * FROM listings WHERE owner_id=%s ORDER BY id DESC", (user_id,)).fetchall()
        conn.close()
        if not rows:
    ...
    if action == "subscriptions":
        ...
        await query.edit_message_text(f"🔔 Ваши подписки ({len(rows)}):", reply_markup=home_ikb())
        ...
        for sub_id, stype, act, lat, lon, radius, created, expires, active in rows:
            ...
            text = (
                ...
                f"⏰ Истекает: {expires[:10] if expires else '—'}\n"
                f"{status}"
            )
            
            btns = []
            if active:
                btns.append(InlineKeyboardButton("⏸ Отключить", callback_data=f"unsub_{sub_id}"))
            else:
                btns.append(InlineKeyboardButton("▶️ Включить", callback_data=f"resub_{sub_id}"))
            btns.append(InlineKeyboardButton("🗑 Удалить", callback_data=f"delsub_{sub_id}"))
...
async def notify_subscribers(ctx, listing_id: int, action: str, ltype: str, lat: float, lon: float, price: float):
    ...
    listing = conn.execute("SELECT * FROM listings WHERE id=%s", (listing_id,)).fetchone()
    conn.close()
    ...
    for user_id, stype, sub_lat, sub_lon, radius, max_price in subscriptions:
        ...
        try:
            ...
            if not purchased and user_id != owner_id and user_id != ADMIN_ID:
                buttons.append([InlineKeyboardButton("📞 Показать контакты (⭐100)", callback_data=f"reveal_{listing_id}")])
...
async def search_radius_chosen(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for row in no_geo:
        ...
        # Без геолокации просто текст
        if row[11]:
            await query.message.reply_photo(row[11], caption=caption, parse_mode="Markdown")
        else:
...
async def show_map(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    row = conn.execute("SELECT lat, lon, address FROM listings WHERE id=%s", (lid,)).fetchone()
    conn.close()
...
async def reveal_contacts(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Обработка кнопки 'Показать контакты' - запуск оплаты через Telegram Stars."""
    query = update.callback_query
    await query.answer()
    lid = int(query.data.split("_")[1])
    ...
    conn = db()
    row = conn.execute("SELECT type, address FROM listings WHERE id=%s", (lid,)).fetchone()
    conn.close()
    ...
    try:
        ...
        await query.answer("⭐ Оплатите через Telegram Stars")
    except Exception as e:
...
async def successful_payment_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if parts[0] == "contacts":
            ...
            # Получаем объявление с контактами
            ...
            if row:
                ...
                await update.message.reply_text(
                    f"✅ *Оплата прошла!* (⭐ {price_stars} Stars)\n\n{caption}",
                    parse_mode="Markdown",
                    reply_markup=keyboard
                )
            else:
                await update.message.reply_text("✅ Оплата прошла, но объявление не найдено.")
...
async def manage_listing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    conn    = db()
    row     = conn.execute("SELECT owner_id FROM listings WHERE id=%s", (lid,)).fetchone()
    if not row or row[0] != user_id:
        await query.answer("Нет доступа", show_alert=True)
        conn.close()
        return MAIN_MENU
    if action == "deactivate":
        conn.execute("UPDATE listings SET active=0 WHERE id=%s", (lid,))
        conn.commit(); await query.answer("⏸ Снято")
    elif action == "activate":
        conn.execute("UPDATE listings SET active=1 WHERE id=%s", (lid,))
        conn.commit(); await query.answer("✅ Активировано")
    elif action == "delete":
        conn.execute("DELETE FROM listings WHERE id=%s", (lid,))
...
async def manage_subscription(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    conn = db()
    row = conn.execute("SELECT user_id FROM search_subscriptions WHERE id=%s", (sub_id,)).fetchone()
    if not row or row[0] != user_id:
        await query.answer("Нет доступа", show_alert=True)
        conn.close()
        return MAIN_MENU
    ...
    if action == "unsub":
        conn.execute("UPDATE search_subscriptions SET active=0 WHERE id=%s", (sub_id,))
        conn.commit()
        await query.answer("⏸ Подписка отключена")
    elif action == "resub":
        conn.execute("UPDATE search_subscriptions SET active=1 WHERE id=%s", (sub_id,))
        conn.commit()
        await query.answer("✅ Подписка включена")
    elif action == "delsub":
        conn.execute("DELETE FROM search_subscriptions WHERE id=%s", (sub_id,))
        conn.commit()
        await query.answer("🗑 Подписка удалена")
        await query.message.reply_text(f"🗑 Подписка #{sub_id} удалена.")
    ...
    for lid, lat, lon, addr in rows:
        ...
        if looks_like_coords:
            ...
            if new_addr:
                ...
                conn.execute("UPDATE listings SET address=%s WHERE id=%s", (new_addr_str, lid))
                fixed += 1
    conn.commit()
    conn.close()
...
async def admin_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("adm_listings_"):
        ...
        conn = db()
        cur  = conn.execute("SELECT active FROM listings WHERE id=%s", (lid,)).fetchone()
        if cur:
            new = 0 if cur[0] else 1
            conn.execute("UPDATE listings SET active=%s WHERE id=%s", (new, lid))
            conn.commit()
