...
def listing_text(row, distance_m=None, viewer_id=None, show_contacts=False):
    ...
    if contacts_visible:
        ...
        # Показываем только первую часть адреса (до запятой) или "Варна"
        short_addr = address.split(',')[0] if ',' in address else "Варна"
...
async def notify_subscribers(ctx, listing_id: int, action: str, ltype: str, lat: float, lon: float, price: float):
    ...
    for user_id, stype, sub_lat, sub_lon, radius, max_price in subscriptions:
        ...
        try:
            caption = listing_text(listing, distance_m=dist, viewer_id=user_id, show_contacts=False)
            notification = (
                f"🔔 *Новое объявление по вашей подписке!*\n\n"
                f"{caption}\n\n"
                f"👉 Используйте /start чтобы посмотреть"
            )
